...
{\color{red}
\subsection{Adaptive Noise Schedule}
\label{sec:method_adaptive}

In the standard DDPM formulation adopted by \textit{DenoiseRep}, the noise schedule $\{\beta_t\}$ is a globally predefined sequence (e.g., linear or cosine) applied uniformly across all layers. However, as evidenced by our layer-wise ablation study in Section~\ref{sec:layer_wise_denoising}, different layers exhibit distinct noise characteristics: deeper layers encode higher-level semantic features where noise contamination is more detrimental, while shallower layers handle finer-grained, lower-level patterns with different noise sensitivity. Applying a single, fixed schedule to all layers ignores this heterogeneity and limits the denoising effectiveness.

To address this limitation, we propose an \textbf{Adaptive Noise Schedule} that learns a layer-specific noise schedule rather than relying on a global predefined one. Concretely, for each denoising layer $n \in \{1, \ldots, N\}$, we introduce a trainable noise schedule parameter $\beta_t^{(n)}$ that replaces the shared $\beta_t$. We parameterize the layer-wise schedule using a lightweight MLP $g_\phi$ that takes the layer index $n$ and the feature statistics of that layer as input:
\begin{equation}
\beta_t^{(n)} = g_\phi\bigl(n,\, \mu^{(n)},\, (\sigma^{(n)})^2\bigr),
\label{eq:adaptive_beta}
\end{equation}
where $\mu^{(n)}$ and $(\sigma^{(n)})^2$ denote the running mean and variance of the backbone features at layer $n$, computed over the training set. The MLP $g_\phi$ consists of two fully connected layers with a sigmoid activation, ensuring the output lies in $(0, 1)$.

With the learned layer-wise schedule, the forward diffusion process at layer $n$ becomes:
\begin{equation}
q(\mathbf{x}_t^{(n)} | \mathbf{x}_{t-1}^{(n)}) = \mathcal{N}\bigl(\mathbf{x}_t^{(n)};\, \sqrt{1 - \beta_t^{(n)}}\,\mathbf{x}_{t-1}^{(n)},\, \beta_t^{(n)}\, \mathbf{I}\bigr),
\label{eq:adaptive_forward}
\end{equation}
which preserves the isotropic Gaussian form but allows each layer to adaptively control its noise magnitude. This design is motivated by Improved DDPM~\cite{nichol2021improved}, which demonstrated that learning the noise schedule significantly improves generation quality; we transfer this insight to the feature denoising setting.

\textbf{Compatibility with parameter fusion.}
Crucially, the adaptive noise schedule preserves the computation-free property of \textit{DenoiseRep\textsuperscript{++}}. Revisiting the fusion formula in Eq.~(\ref{eq:One-step denoising (combined)}), the layer-wise $\beta_t^{(n)}$ only modifies the coefficients $C_1(t)$ and $C_2(t)$, which are scalar constants determined once after training. The fused weight matrix $W'^{(n)} = [W - C_1^{(n)}(t)\, W\, W_D]$ and bias term $b'^{(n)}$ can still be precomputed and merged into the backbone parameters. Therefore, the adaptive schedule introduces no additional inference cost and remains fully compatible with the linear parameter fusion framework.


\subsection{Progressive Denoising Training}
\label{sec:method_progressive}

The standard training protocol of \textit{DenoiseRep} trains all denoising layers simultaneously with their respective noise levels, where each layer $i$ is assigned a fixed diffusion step $t = i$. However, this joint optimization may lead to training instability, particularly for shallow layers that handle subtle noise and are sensitive to the cascading errors from imperfectly trained deeper layers.

Inspired by curriculum learning~\cite{bengio2009curriculum}, we propose a \textbf{Progressive Denoising Training} strategy that trains the denoising modules in a coarse-to-fine manner. The key idea is to first train the deeper layers (which handle stronger noise and yield larger performance gains, as evidenced by our layer-wise analysis in Section~\ref{sec:layer_wise_denoising}), and then progressively unlock shallower layers:

\textbf{Phase 1 (Deep layers):} Only the denoising modules at layers $\lfloor 2N/3 \rfloor$ to $N$ are active and trained, while shallower modules remain frozen at initialization. This allows the model to first learn the coarse-grained denoising of high-level semantic noise.

\textbf{Phase 2 (Middle layers):} The deep-layer modules are kept trainable, and layers $\lfloor N/3 \rfloor$ to $\lfloor 2N/3 \rfloor$ are additionally activated. The training continues with a reduced learning rate for the deep layers.

\textbf{Phase 3 (All layers):} All denoising modules are jointly fine-tuned with a warm-up from the previous phases, enabling end-to-end refinement of the complete denoising trajectory.

Formally, at training phase $p \in \{1, 2, 3\}$, the active layer set $\mathcal{S}_p$ and the training objective are:
\begin{equation}
\mathcal{L}_{\text{prog}}^{(p)} = \sum_{i \in \mathcal{S}_p} \left\| \epsilon_i - D_{\theta_i}(X_{t_i}, t_i) \right\|_2^2,
\label{eq:progressive_loss}
\end{equation}
where $\mathcal{S}_1 = \{\lfloor 2N/3 \rfloor, \ldots, N\}$, $\mathcal{S}_2 = \{\lfloor N/3 \rfloor, \ldots, N\}$, and $\mathcal{S}_3 = \{1, \ldots, N\}$.

This progressive schedule offers two advantages: (1) it ensures that the most impactful layers (deeper layers) converge first, providing a stable foundation for subsequent layers; and (2) it prevents premature optimization of shallow layers that depend on accurate denoising signals from deeper stages, thereby reducing error accumulation across the denoising chain.
}% end red
...
\subsection{Effect of Adaptive Noise Schedule and Progressive Training}
\label{sec:exp_adaptive_progressive}

We conduct ablation experiments to evaluate the contributions of the proposed Adaptive Noise Schedule (ANS) and Progressive Denoising Training (PDT) strategies. All experiments are performed on DukeMTMC-reID with TransReID-SSL (ViT-Small backbone), and we report both mAP and Rank-1 accuracy.

\begin{table}[t]
\centering
\caption{Ablation study of Adaptive Noise Schedule (ANS) and Progressive Denoising Training (PDT) on DukeMTMC-reID. Backbone: ViT-Small.}
\label{tab:ablation_ans_pdt}
\vspace{0.2cm}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{l|cc}
\hline \hline
\textbf{Method} & \textbf{mAP} & \textbf{Rank-1} \\
\hline
Baseline (TransReID-SSL) & 81.2\% & 87.8\% \\
+ \textit{DenoiseRep\textsuperscript{++}} & 82.6\% & 88.7\% \\
\hline
+ \textit{DenoiseRep\textsuperscript{++}} + ANS & 82.8\% & 89.0\% \\
+ \textit{DenoiseRep\textsuperscript{++}} + PDT & 83.1\% & 89.1\% \\
+ \textit{DenoiseRep\textsuperscript{++}} + ANS + PDT & 83.3\% & 89.3\% \\
\hline \hline
\end{tabular}
\end{table}

As shown in Table~\ref{tab:ablation_ans_pdt}, both proposed strategies yield consistent improvements over the \textit{DenoiseRep\textsuperscript{++}} baseline.

\textbf{Effect of Adaptive Noise Schedule.}
By replacing the fixed global noise schedule with the proposed layer-wise learnable variant (Section~\ref{sec:method_adaptive}), each layer adapts its noise magnitude according to its feature statistics. This allows deeper layers, which encode higher-level semantics and are more susceptible to noise, to adopt stronger denoising, while shallower layers with less noise contamination apply milder schedules. As a result, mAP improves from 82.6\% to 82.8\%, demonstrating that accounting for the layer-wise heterogeneity of noise distributions is beneficial.

\textbf{Effect of Progressive Denoising Training.}
The progressive training strategy (Section~\ref{sec:method_progressive}) trains deeper denoising layers first before gradually activating shallower ones. This curriculum-style approach provides a more stable optimization trajectory: deeper layers, which handle stronger noise, converge first and provide reliable denoised features as input to subsequent layers. This reduces the cascading error accumulation observed in joint training. Compared to the standard \textit{DenoiseRep\textsuperscript{++}} training, PDT achieves 83.1\% mAP, indicating that training order matters for the denoising chain.

\textbf{Combined effect.}
When both ANS and PDT are applied together, the model achieves 83.3\% mAP and 89.3\% Rank-1 accuracy. The two strategies are complementary: ANS improves \textit{what} noise schedule is applied (layer-aware noise magnitude), while PDT improves \textit{how} the denoising modules are optimized (stable training dynamics). Their combination addresses both the noise modeling and optimization aspects of feature-level denoising, leading to the strongest overall performance.
}% end red
...
\hline
SimCSE-BERT-base + \textit{\textbf{DenoiseRep\textsuperscript{++}}} (unsupervised) & \textbf{77.11} \\
SimCSE-BERT-base + \textit{\textbf{DenoiseRep\textsuperscript{++}}} (supervised) & \textbf{81.89} \\
\hline \hline
\end{tabular}
\end{table}
...
\begin{figure*}[t]
    ...
    \begin{minipage}{0.96\linewidth}
        ...
        % 最后的 \hfill 确保图片均匀分布
        \subfloat[]{\includegraphics[width=0.19\linewidth]{picture/det_vis/base_01.jpg}}\hfill
        \subfloat[]{\includegraphics[width=0.19\linewidth]{picture/det_vis/base_03.jpg}}\hfill
        \subfloat[]{\includegraphics[width=0.19\linewidth]{picture/det_vis/base_04.jpg}}\hfill
        \subfloat[]{\includegraphics[width=0.19\linewidth]{picture/det_vis/base_05.jpg}}\hfill
        \subfloat[]{\includegraphics[width=0.19\linewidth]{picture/det_vis/base_06.jpg}}
    \end{minipage}
    ...
    \begin{minipage}{0.96\linewidth}
        \centering
        \subfloat[]{\includegraphics[width=0.19\linewidth]{picture/det_vis/denoiserep_01.jpg}}\hfill
        \subfloat[]{\includegraphics[width=0.19\linewidth]{picture/det_vis/denoiserep_03.jpg}}\hfill
        \subfloat[]{\includegraphics[width=0.19\linewidth]{picture/det_vis/denoiserep_04.jpg}}\hfill
        \subfloat[]{\includegraphics[width=0.19\linewidth]{picture/det_vis/denoiserep_05.jpg}}\hfill
        \subfloat[]{\includegraphics[width=0.19\linewidth]{picture/det_vis/denoiserep_06.jpg}}
    \end{minipage}
