...
如第一章所述，判别式视觉模型通过骨干网络逐层提取特征表示，并在此基础上完成分类、检测、分割和检索等下游任务\cite{bengio2013representation}。然而，在实际推理过程中，骨干网络输出的特征向量中不可避免地包含来自多种来源的噪声：一方面，输入图像本身可能受到光照变化、遮挡、背景杂乱和传感器噪声等因素的干扰；另一方面，特征在网络逐层传播的过程中也会累积表示噪声，导致最终特征的类内分布发散、类间边界模糊，进而影响下游任务的判别精度。
...
在目标检测任务中，选取COCO\cite{lin2014microsoft}数据集。COCO是目标检测领域最常用的大规模基准，包含80个目标类别和超过11万张训练图像。评价指标采用平均精度（AP）及其在不同IoU阈值下的变体AP$_{50}$和AP$_{75}$。
...
前述实验验证了本章方法在多种任务上的有效性，本节进一步从去噪深度和去噪步数两个维度，系统分析不同去噪配置策略对最终性能的影响规律，为实际应用中的超参数选择提供指导。

\subsubsection{逐层去噪位置与步数的影响}

为研究去噪层的位置和每层去噪步数对特征增强效果的联合影响，本节以ViT作为骨干网络，将其12个Transformer编码层划分为三个等距区间：浅层（第1--4层）、中层（第5--8层）和深层（第9--12层），分别在各区间内施加不同步数（1步、2步、4步和8步）的去噪操作。所有实验在DukeMTMC-reID数据集上进行评估，采用mAP作为评价指标，分别使用ViT-Small和ViT-Base两种规模的骨干网络。

\begin{table}[htbp]
\centering
\caption{去噪步数与层区间对行人重识别性能（mAP, \%）的影响（DukeMTMC-reID数据集）\protect\\[-2pt]{\rm Table\;\thetable\quad Effect of denoising step count and layer interval on ReID performance (mAP, \%) on DukeMTMC-reID}}\label{tab:chap3_layer_config}
\vspace{0.2cm}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{c|ccc|ccc}
\hline\hline
\multirow{2}{*}{去噪步数} & \multicolumn{3}{c|}{ViT-Small} & \multicolumn{3}{c}{ViT-Base} \\
 & 1--4层 & 5--8层 & 9--12层 & 1--4层 & 5--8层 & 9--12层 \\
\hline
1步 & 81.5 & 81.4 & 81.5 & 84.1 & 84.1 & 84.1 \\
2步 & 81.7 & 81.9 & 81.9 & 84.2 & 84.1 & 84.2 \\
4步 & 82.2 & 82.3 & 82.5 & 84.1 & 84.1 & 84.3 \\
8步 & 81.8 & 82.0 & 82.2 & 83.7 & 83.9 & 84.0 \\
\hline\hline
\end{tabular}
\end{table}

从表\ref{tab:chap3_layer_config}可以得出以下三个重要的规律性发现：

（1）深层特征从去噪中获益更大。在ViT-Small和ViT-Base两种骨干网络上，对深层区间（第9--12层）施加去噪一致取得了最优性能。以ViT-Small上的4步去噪为例，深层区间的mAP达到82.5\%，而浅层区间为82.2\%、中层区间为82.3\%。这一观察与直觉相符：深层编码更高层次的语义特征，这些特征中的噪声对判别性的影响更为直接和关键，因此去噪在深层产生的增益更加显著。浅层特征主要编码低级视觉模式（如边缘和纹理），这些特征中的噪声对最终判别性能的影响相对较小。

（2）适度的去噪步数是最优选择。随着去噪步数从1步增加到4步，性能普遍单调提升；但当步数进一步增加到8步时，性能反而出现回落。以ViT-Small的深层区间为例，mAP从1步的81.5\%逐步上升到4步的82.5\%，但8步时下降至82.2\%。这一现象表明，过度去噪可能会过度平滑特征表示，抹去部分具有判别意义的细微信号，导致信息损失。4步去噪在噪声去除和特征保持之间取得了较好的平衡，是推荐的默认配置。

（3）大规模骨干网络对去噪配置更加鲁棒。ViT-Base在所有配置下的性能变化范围（83.7\%至84.3\%，0.6\%跨度）显著小于ViT-Small（81.4\%至82.5\%，1.1\%跨度）。这表明参数量更大的模型本身提取的特征更加鲁棒，对去噪超参数的敏感度较低。然而，即使是大规模骨干网络，在适当的配置下（如深层4步）仍然能够从去噪增强中获得一致的性能改善。这一发现具有实际指导价值：对于小规模模型，需要更仔细地调节去噪配置以获得最佳增益；对于大规模模型，则可以使用较为宽松的配置而无需精细调优。

\subsubsection{去噪强度的任务适应性分析}

不同的判别式任务对特征中噪声的敏感程度存在差异，这决定了最优去噪强度的选择。本节在图像分类（ImageNet-1k，SwinV2-T）和行人重识别（DukeMTMC-reID，ViT-Small）两个代表性任务上，分析不同扩散步数$T$对性能的影响。

\begin{table}[htbp]
\centering
\caption{不同扩散步数$T$对两类任务性能的影响\protect\\[-2pt]{\rm Table\;\thetable\quad Effect of diffusion step $T$ on task performance}}\label{tab:chap3_step_T}
\vspace{0.2cm}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{c|cc}
\hline\hline
扩散步数$T$ & 分类（acc@1） & 检索（mAP） \\
\hline
100 & 81.91\% & 81.42\% \\
500 & 82.03\% & 81.78\% \\
1000 & 82.13\% & 82.12\% \\
...
从表\ref{tab:chap3_step_T}可以观察到：两个任务在$T=1000$时均达到性能峰值。当$T$过小（如100）时，噪声调度过于粗糙，去噪模块难以精确学习噪声分布的细粒度结构；当$T$过大（如2000）时，训练过程中的噪声累积加剧，导致去噪模块的收敛变得困难。$T=1000$在精度与训练稳定性之间提供了良好的折中。此外，行人重识别任务对$T$的敏感度高于分类任务（性能变化范围0.70\% vs. 0.22\%），这与检索任务对特征细粒度判别性的更高要求一致。综合以上分析，本文在所有实验中统一采用$T=1000$的设置。
